...
function buildUseContextMenuTrigger(triggerVisible) {
  return _config => {
    ...
    const mouseDownTimeoutId = react.useRef();
    const touchstartTimeoutId = react.useRef();
...
const useContextMenu = ({
  ...
  const selectables = react.useRef([]);
  const [style, setStyles] = react.useState(baseStyles);
  const [selectedIndex, setSelectedIndex] = react.useState(-1);
  const [isVisible, setVisible] = react.useState(false);
  const [coords, setCoords] = react.useState([0, 0]);
  const [collectedData, setCollectedData] = react.useState();
  const hideMenu = react.useCallback(() => setVisible(false), [setVisible]);
  const triggerVisible = react.useCallback((coords, data) => {
    setVisible(true);
    setCoords(coords);
